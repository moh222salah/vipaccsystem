...
if (isset($_POST['setprefs'])) 
{
	if (!is_numeric($_POST['query_size']) || ($_POST['query_size']<1))
	{
		...
		if ($chg_theme || $chg_lang || $chg_date_format || $chg_date_sep) {
			// After language/theme change, return to the page user came from (stored in HTTP_REFERER)
			// or fall back to display_prefs itself
			if ($chg_lang && isset($_POST['_return_url']) && !empty($_POST['_return_url'])) {
				meta_forward(htmlspecialchars_decode($_POST['_return_url']));
			} else {
